...
template_nodes = [Supernode(i, [str(i)]) for i in equiv_classes]
...
def create_partial_match(n):
    ...
    for i in range(n):
        partial_match.add_match(SuperTemplateNode(equiv_classes[i]), Supernode(new_match[i], [str(i)]))
