...
from uclasmcode.uclasm.utils import data_structures
import numpy as np
...
class Graph:
    def __init__(self, nodes, channels, adjs, labels=None):
        ...
        if type(next(iter(adjs))) is np.ndarray:
            self.ch_to_adj = {ch: adj for ch, adj in zip(channels, adjs)}
        else:
            self.ch_to_adj = {ch: adj.A for ch, adj in zip(channels, adjs)}
    ...
    def nbr_idx_pairs(self):
        ...
        """
        return np.argwhere(np.tril(self.is_nbr))
...
def sparse_to_dense_graph(old_graph: data_structures.Graph):
    result = Graph(old_graph.nodes, old_graph.channels, old_graph.adjs, old_graph.labels)
    return result
