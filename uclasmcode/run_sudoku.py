...
def display_sudoku(show_cands=False):
    if not show_cands:
...
total_filter_time = 0
total_iso_count_time = 0
with open("sudoku-easy50.txt", encoding="utf-8") as fin:
...
#        tmplt.summarize_candidate_sets()
        ...
        start_time = default_timer()
        tmplt, world, candidates = uclasm.run_filters(tmplt, world, candidates=tmplt.is_cand, init_changed_cands=changed_nodes, filters=uclasm.all_filters, verbose=True)
        print("Time taken for filters: {}".format(default_timer()-start_time))
        total_filter_time += default_timer()-start_time
#        display_sudoku(show_cands=True)
        ...
        total_iso_count_time += default_timer()-start_time
        raise Exception()
print("Total time: {}".format(default_timer()-total_start_time))
print("Time spent filtering: {}".format(total_filter_time))
print("Time spent counting isomorphisms: {}".format(total_iso_count_time))
