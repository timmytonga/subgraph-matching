...
from uclasmcode.candidate_structure.logging_utils import print_debug
...
def topology_filter(tmplt, world, candidates, *,
	...
	"""
	print_debug(f"In topology filter")
	for src_idx, dst_idx in tmplt.nbr_idx_pairs:
		...
		if not any(src_matches):
			candidates[:, :] = False
			break
		print_debug("BEFORE: src_idx != dst_idx ")

		if src_idx != dst_idx:
			# dsts with at least one reasonable src
			dst_matches = np.count_nonzero(enough_edges, axis=0) > 0
			candidates[dst_idx][dst_is_cand] = dst_matches
			if not any(dst_matches):
				candidates[:, :] = False
				break

	print_debug("Finished topology filter")
