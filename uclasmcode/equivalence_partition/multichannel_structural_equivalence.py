...
def combine_channel_equivalence(equiv_partition_dict):
    ...
    """
    onePartition = next(iter(equiv_partition_dict.values()))
    numberOfElements = len(onePartition) # the __len__ implementation of Equivalence returns total number of element

    result = Equivalence(range(numberOfElements))
...
def partition_multichannel(ch_to_adj) -> Equivalence:
    ...
    for ch, sparse_mat in ch_to_adj.items():
        ...
        equiv_partition_dictionary[ch] = partition_vertices(adj_matrix)  # compute the partition for that channel
